...
func TestSensorBeginAndEndAroundAFallingBox(t *testing.T) {
	...
	thin := QMustParse("0.0005")
	_, sensorShapeId := addSensorBox(t, worldId, Vec2Zero(), QOne(), thin)
	_, visitorShapeId := addSensorVisitorBox(t, worldId, Vec2{Y: QMustParse("0.0012")}, QHalf(), thin, true)
...
func TestSensorEventsAreOrderedByShapeId(t *testing.T) {
	worldId := createTestWorld(t)
	_, sensorShapeId := addSensorBox(t, worldId, Vec2Zero(), QFromInt(4), QFromInt(2))
	_, visitorA := addSensorVisitorBox(t, worldId, v2(2, 0), QHalf(), QHalf(), true)
	_, visitorB := addSensorVisitorBox(t, worldId, v2(-2, 0), QHalf(), QHalf(), true)
...
func TestSensorIgnoresVisitorWithoutSensorEvents(t *testing.T) {
	...
	addSensorVisitorBox(t, worldId, Vec2Zero(), QHalf(), QHalf(), false)
...
func TestDestroyVisitorEmitsEnd(t *testing.T) {
	worldId := createTestWorld(t)
	_, sensorShapeId := addSensorBox(t, worldId, Vec2Zero(), QFromInt(4), QFromInt(2))
	_, visitorShapeId := addSensorVisitorBox(t, worldId, Vec2Zero(), QHalf(), QHalf(), true)
...
func TestDestroySensorEmitsEnds(t *testing.T) {
	worldId := createTestWorld(t)
	sensorBodyId, sensorShapeId := addSensorBox(t, worldId, Vec2Zero(), QFromInt(4), QFromInt(2))
	_, visitorA := addSensorVisitorBox(t, worldId, v2(2, 0), QHalf(), QHalf(), true)
	_, visitorB := addSensorVisitorBox(t, worldId, v2(-2, 0), QHalf(), QHalf(), true)
...
func TestSensorTouchingEdgeIsNotOverlap(t *testing.T) {
	worldId := createTestWorld(t)
	addSensorBox(t, worldId, Vec2Zero(), QOne(), QOne())
	visitorX := QFromInt(2).Add(linearSlop)
	addSensorVisitorBox(t, worldId, Vec2{X: visitorX}, QOne(), QOne(), true)
