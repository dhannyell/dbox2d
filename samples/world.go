...
func NewLargeWorld(ctx *SampleContext) Sample {
...

	{
		...
		gridSize := dbox2d.F(largeWorldGridSize)
		halfExtent := dbox2d.F(0.4).Mul(gridSize)
		...
		for i := range largeWorldGridCount {
			...
			if i%10 == 0 {
				bodyDef.Position.X = dbox2d.F(xBody)
			...
			for range ycount {
				square := dbox2d.MakeOffsetBox(halfExtent, halfExtent, dbox2d.Vec2{X: dbox2d.F(xShape), Y: y}, dbox2d.RotIdentity())
				square.Radius = dbox2d.F(0.1)
	...
	for cycleIndex := range largeWorldCycleCount {
		...
		xbase := dbox2d.F(largeWorldPeriod/2 + cycleIndex*largeWorldPeriod + xStart)

		remainder := cycleIndex % 3
		if remainder == 0 {
			bodyDef := dbox2d.DefaultBodyDef()
			bodyDef.Type = dbox2d.DynamicBody
			bodyDef.Position = dbox2d.Vec2{X: xbase.Sub(dbox2d.F(3)), Y: dbox2d.F(10)}

			shapeDef := dbox2d.DefaultShapeDef()
			box := dbox2d.MakeBox(dbox2d.F(0.3), dbox2d.F(0.2))

			for range 10 {
				bodyDef.Position.Y = dbox2d.F(10)
				for range 5 {
				...
				bodyDef.Position.X = bodyDef.Position.X.Add(dbox2d.F(0.6))
			}
		} else if remainder == 1 {
			position := dbox2d.Vec2{X: xbase.Sub(dbox2d.F(2)), Y: dbox2d.F(10)}
			for range 5 {
				shared.CreateHuman(s.WorldId, position, dbox2d.F(1.5), dbox2d.F(0.05), dbox2d.QZero(), dbox2d.QZero(), humanIndex+1, nil, false)
				humanIndex += 1
				position.X = position.X.Add(dbox2d.QOne())
			}
		} else {
			position := dbox2d.Vec2{X: xbase.Sub(dbox2d.F(4)), Y: dbox2d.F(12)}

			for range 5 {
				var d donut
				d.create(s.WorldId, position, dbox2d.F(0.75), 0, false, nil)
				position.X = position.X.Add(dbox2d.F(2))
			}
		}
	}

	s.car.spawn(s.WorldId, dbox2d.Vec2{X: dbox2d.F(xStart + 20), Y: dbox2d.F(40)},
		dbox2d.F(10), dbox2d.F(2), dbox2d.F(0.7), dbox2d.F(2000), nil)

	s.cycleIndex = 0
	s.speed = 0
	s.explosionPosition = dbox2d.Vec2{
		X: dbox2d.F(largeWorldPeriod/2 + s.cycleIndex*largeWorldPeriod + xStart),
		Y: dbox2d.F(7),
	}
	s.explode = true
	s.followCar = false
...
func (s *LargeWorld) Step() {
	...
	if s.StepCount&0x1 == 0x1 && s.explode {
		s.explosionPosition.X = dbox2d.F(largeWorldPeriod/2 + s.cycleIndex*largeWorldPeriod - span)
