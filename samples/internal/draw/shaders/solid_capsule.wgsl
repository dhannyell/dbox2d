...
// Ported from samples/data/solid_capsule.vs and samples/data/solid_capsule.fs of Box2D v3.1.1
//
// Vertex buffer 0 (step_mode: vertex): the unit quad, @location(0), the
// six vertices of draw.QuadVertices.
// Vertex buffer 1 (step_mode: instance), matching CapsuleData:
//   @location(1) transform: vec4<f32> (x, y, cos, sin) offset 0
//   @location(2) radius:    f32                          offset 16
//   @location(3) length:    f32                           offset 20
//   @location(4) color:     unorm8x4                      offset 24
//
...
struct VertexInput {
    ...
    @location(3) instance_length: f32,
...
struct VertexOutput {
    ...
    @location(2) f_length: f32,
    @location(3) f_thickness: f32,
}
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    var p = vec2<f32>(scale * in.local_position.x, scale * in.local_position.y);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let radius = 0.5 * (2.0 - in.f_length);

    let border_color = in.f_color;
    let fill_color = 0.6 * border_color;

    let v1 = vec2<f32>(-0.5 * in.f_length, 0.0);
    let v2 = vec2<f32>(0.5 * in.f_length, 0.0);

    let e = v2 - v1;
    let w = in.f_position - v1;
    let we = dot(w, e);
    let b = w - e * clamp(we / dot(e, e), 0.0, 1.0);
    let dw = length(b);

    let d = min(dw, abs(dw - radius));
